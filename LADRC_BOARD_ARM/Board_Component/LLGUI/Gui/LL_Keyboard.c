...
#define DEF_ID_KEYBOARD_KEEP_OUT        0xFF2A
...
bool slotKeyboardNumClose(llConnectInfo info)
{
    ...
    nWindowDelete(DEF_ID_KEYBOARD_KEEP_OUT);
...
bool slotKeyboardNumEntry(llConnectInfo info)
{
    ...
    pLineEditNumCheckRange((llLineEdit*)(targetWidget));
...
void llKeyboardNumCreate(void* targetWidget, int16_t x, int16_t y, int16_t width, int16_t height,llFontLib *fontLib)
{
    ...
    winKeepOut=llWindowCreate(DEF_ID_KEYBOARD_KEEP_OUT,rootWidget->nameId,0,0,width,keyboardY,false);
    
    if((window!=NULL)&&(userInfo!=NULL)&&(winKeepOut!=NULL))
    {
