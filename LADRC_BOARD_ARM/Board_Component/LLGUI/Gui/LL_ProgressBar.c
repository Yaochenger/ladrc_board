...
void pProgressBarRefresh(llProgressBar *widget)
{
    ...
    if((widget->isEnable)&&(!widget->isHidden))
    {
        if(llGeneralParentLinkHidden((llGeneral*)widget)==false)
        {
            ...
            if(widget->isShowPercent)
            {
                width=widget->geometry.width-widget->textInfo.geometry.width-2;
            }
            else
            {
                width=widget->geometry.width-2;
            }
            ...
            {
                x1=x+u16Temp;
                if(x1>(x+width-1))//防止超范围
                {
                    x1=x+width-1;
                }
                llFillSingleColor(x+widget->percentWidth,y,x1,y+height-1,widget->progressColor);
            }
            else
            {
                x1=x+widget->percentWidth;
                if(x1>(x+width-1))//防止超范围
                {
                    x1=x+width-1;
                }
                if(widget->percentWidth>u16Temp)//差值,刷新
                {
                    llFillSingleColor(x+u16Temp,y,x1,y+height-1,widget->backgroundColor);
                }
                else//相同值,刷新
                {
                    llFillSingleColor(x,y,x1,y+height-1,widget->progressColor);
                }
            }
...
llProgressBar *llProgressBarQuickCreate(uint16_t nameId, uint16_t parentNameId, int16_t x, int16_t y, int16_t width, int16_t height,
                                        ...
                                        uint8_t percent,bool isShowPercent,bool isHidden)
{
    ...
    if(llList_GetInfoByName(&parentInfo,parentNameId)==true)
    {
        ...
        if((pNewWidget!=NULL)&&(pText!=NULL))
        {
            ...
            pNewWidget->isShowPercent=isShowPercent;
...
void pProgressBarSetShowPercent(llProgressBar *widget, bool state)
{
    llPoint globalPos;
    widget->isShowPercent=state;
    if(state==false)
    {
        globalPos=llListGetGlobalPos(widget->parentWidget);
        widget->textInfo.geometry.x=globalPos.x+widget->geometry.x+widget->geometry.width-widget->textInfo.fontLibInfo->fontSize*4;
...
void nProgressBarSetShowPercent(uint16_t nameId, bool state)
{
    void *widget;
    widget=llGeneralGetWidget(nameId);
    if(widget!=NULL)
    {
        pProgressBarSetShowPercent((llProgressBar*)widget,state);
    }
}
