...
llBackground *llBackgroundQuickCreate(uint16_t nameId,int16_t width,int16_t height,bool isColor,llColor color,uint32_t imageAddr)
{
    ...
    if(pNewWidget!=NULL)
    {
        ...
        if(isColor)
        {
            pBackgroundSetColor(pNewWidget,pNewWidget->color);
        }
        else
        {
            pBackgroundSetImage(pNewWidget,pNewWidget->imageAddr);
        }
        //add linked list
...
{
    return llBackgroundQuickCreate(nameId,LL_MONITOR_WIDTH,LL_MONITOR_HEIGHT,true,RGB_CONVERT(240,240,240),0);
}
...
void nBackgroundDelete(uint16_t nameId)
{
    ...
    if(llList_GetInfoByName(&linkInfo,nameId)==true)
    {
        ...
        list_for_each_prev_safe(tempPos, safePos, &linkInfo->child_link)
        {
            ...
            //清除connect链表
            if(((llGeneral *)tempInfo->widget)->widgetType!=widgetTypeWindow)
            {
...
uint16_t llBackgroundGetId(void)
{
    return activeBgId;
}
