    ...
    if((widget->isEnable)&&(!widget->isHidden))
    {
        if(llGeneralParentLinkHidden((llGeneral*)widget)==false)
        {
            ...
            if(addr)
            {
                ...
                year=year<<8;
                year=year+rtcBuf[1];
                ret=snprintf(tempBuf,5,"%04d",year);
                if(ret > 0)
                {
            ...
            if(addr)
            {
                ret=snprintf(tempBuf,3,"%02d",rtcBuf[2]);
                if(ret > 0)
                {
            ...
            if(addr)
            {
                ret=snprintf(tempBuf,3,"%02d",rtcBuf[3]);
                if(ret > 0)
                {
                    memcpy(addr,tempBuf,2);
                }
            ...
            if(addr)
            {
                ret=snprintf(tempBuf,3,"%02d",rtcBuf[4]);
                if(ret > 0)
                {
                    memcpy(addr,tempBuf,2);
                }
            ...
            if(addr)
            {
                ret=snprintf(tempBuf,3,"%02d",rtcBuf[5]);
                if(ret > 0)
                {
                    memcpy(addr,tempBuf,2);
                }
            ...
            if(addr)
            {
                ret=snprintf(tempBuf,3,"%02d",rtcBuf[6]);
                if(ret > 0)
                {
                    memcpy(addr,tempBuf,2);
                }
...
//            }
            ...
            llCharDisplay((llGeneral*)widget,&(widget->textInfo),true);
        }
    }
}
...
void pDateTimeRecoverBackground(llDateTime *widget,llGeometry geometry)
{
    ...
    {
        llFillSingleColor(geometry.x,geometry.y,geometry.x+geometry.width-1,geometry.y+geometry.height-1,widget->textInfo.backgroundColor);
    }
}
