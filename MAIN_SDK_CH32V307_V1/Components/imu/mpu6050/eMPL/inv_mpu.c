...
/* Platform adaptation declarations. */
...
/* Board orientation matrix. */
static signed char gyro_orientation[9] = { 
	                                         1, 0, 0,
                                           0, 1, 0,
                                           0, 0, 1
                                         };
/* Run built-in self test and apply gyro bias to DMP. */
...
/* Convert orientation matrix row to encoded scale value. */
...
/* Convert one orientation matrix row to scalar code. */
...
unsigned char mpu_dmp_init(void)
{
	...
	if(mpu_init()==0) // Initialize MPU6050 core.
	{
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL); // Enable gyro and accel.
		if(res)return 1;
		res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL); // Configure FIFO sources.
		if(res)return 2;
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ); // Set sample rate.
		if(res)return 3;
		res=dmp_load_motion_driver_firmware(); // Load DMP firmware.
		if(res)return 4;
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)); // Set board orientation.
		if(res)return 5;
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP| // Enable required DMP features.
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6;
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ); // Set DMP FIFO rate.
		if(res)return 7;
		res=run_self_test(); // Run self-test.
		if(res)return 8;
		res=mpu_set_dmp_state(1); // Enable DMP output.
		if(res)return 9;
	}else return 10;
...
/* Get Euler angles from DMP quaternion output. */
/* pitch range: about -90 to +90 degrees */
/* roll  range: about -180 to +180 degrees */
/* yaw   range: about -180 to +180 degrees */
/* Return 0 on success. */
unsigned char mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	...
	{
		q0 = quat[0] / q30;	// Convert q30 fixed-point quaternion to float.
		q1 = quat[1] / q30;
		q2 = quat[2] / q30;
		q3 = quat[3] / q30;
