...
void llTextAction(void *widget,uint8_t touchSignal)
{
    ...
    if(((llText*)widget)->isEnable)
    {
        ...
        case SIGNAL_CLICK_PRESSED:
        {
            ...
            {
                if(llClickGetPoint(&pos.x,&pos.y)==true)
                {
...
//            {
//                llFillSingleColor(globalPos.x+widget->geometry.x,globalPos.y+widget->geometry.y,globalPos.x+widget->geometry.x+widget->geometry.width-1,globalPos.y+widget->geometry.y+widget->geometry.height-1,widget->textInfo.backgroundColor);
//            }
            ...
            widget->sysGeometry=llCharDisplay((llGeneral*)widget,&(widget->textInfo),true);
        }
    }
}
...
void pTextMove(llText *widget,int16_t x, int16_t y)
{
...
void nTextMove(uint16_t nameId,int16_t x, int16_t y)
{
    void *widget;
    widget=llGeneralGetWidget(nameId);
    if(widget!=NULL)
    {
        pTextMove(widget,x,y);
    }
}
