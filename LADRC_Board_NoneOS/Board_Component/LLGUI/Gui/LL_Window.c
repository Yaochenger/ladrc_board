...
llWindow *llWindowQuickCreate(uint16_t nameId, uint16_t parentNameId, int16_t x, int16_t y, int16_t width, int16_t height,
                              bool isColor,llColor bgColor,uint32_t imageAddr,
...
{
    return llWindowQuickCreate(nameId,parentNameId,x,y,width,height,false,0,0,true,isHidden);
}
...
void pWindowSpecificAreaRecover(llWindow *widget,llGeometry geometry)
{
    ...
    else
    {
        ...
        geometry.x+=globalPos.x;
        geometry.y+=globalPos.y;
        //需要验证
        if(widget->isColor)
        {
            llFillSingleColor(geometry.x,geometry.y,geometry.x+geometry.width-1,geometry.y+geometry.height-1,widget->color);
        }
        else
        {
            originGeometry.x=globalPos.x;
            originGeometry.y=globalPos.y;
            originGeometry.width=widget->geometry.width;
            originGeometry.height=widget->geometry.height;
            llGeneralImageSpecificAreaShow((llGeneral*)widget,originGeometry,geometry,widget->imageAddr);
        }
    }
}
    ...
    if(widget->isHidden==false)
    {
        if(llGeneralParentLinkHidden((llGeneral*)widget)==false)
        {
            ...
            if(widget->isTransparent==false)
            {
                ...
                {
                    pWindowSetImage(widget,widget->imageAddr);
                }
            }
...
void pWindowSetImage(llWindow *widget,uint32_t imageAddr)
{
    llPoint globalPos;
    globalPos=llListGetGlobalPos(widget->parentWidget);
    widget->isTransparent=false;
    ...
    llGeneralImageShow((llGeneral*)widget,widget->imageAddr,widget->geometry.x+globalPos.x,widget->geometry.y+globalPos.y);
}

void nWindowSetImage(uint16_t nameId,uint32_t imageAddr)
{
    llListWidgetInfo *info;
    if(llList_GetInfoByName(&info,nameId)==true)
    {
        if(((llGeneral*)(info->widget))->widgetType==widgetTypeWindow)
        {
            pWindowSetImage(info->widget,imageAddr);
        }
    }
}
