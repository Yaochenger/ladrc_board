...
//    int16_t offsetX = 0,offsetY = 0;
    ...
    if((widget->isEnable)&&(!widget->isHidden))
    {
        if(llGeneralParentLinkHidden((llGeneral*)widget)==false)
        {
            ...
            {
                llFillSingleColor(globalX+1,globalY+1,globalX+width-2,globalY+height-2,widget->textInfo.backgroundColor);
...
void pLineEditNumCheckRange(llLineEdit *widget)
{
    float fTemp;
    uint8_t buf[16]={0};
    if((widget->isEnable)&&(widget->isNum))
    {
        if(widget->textInfo.text[0]==0)
        {
            widget->textInfo.text[0]='0';
            widget->textInfo.text[1]=0;
            pLineEditRefresh(widget);
        }
        fTemp=llStrToFloat(widget->textInfo.text);
        if(fTemp<widget->minValue)
        {
            strcpy((char*)widget->textInfo.text,(char*)llFloatToStr(widget->minValue,0,buf));
            pLineEditRefresh(widget);
        }
        else
        {
            if(fTemp>widget->maxValue)
            {
                strcpy((char*)widget->textInfo.text,(char*)llFloatToStr(widget->maxValue,0,buf));
                pLineEditRefresh(widget);
            }
        }
    }
