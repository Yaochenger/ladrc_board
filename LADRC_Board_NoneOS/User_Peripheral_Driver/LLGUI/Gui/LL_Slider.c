...
void pSliderRefresh(llSlider *widget)
{
    ...
    llPoint pos;
    uint16_t offset;
    llGeometry bgGeometry,curGeometry,newCurGeometry;
        ...
        if(llGeneralParentLinkHidden((llGeneral*)widget)==false)
        {
            ...
            if(widget->isHorizontal==true)//水平
            {
                if(widget->isColor)
                {
                    ...
                    if((offset+widget->cursorWidth)<widget->geometry.width)
                    {
                        //显示右侧
            ...
            else
            {
                if(widget->isColor)
                {
                    ...
                    if((offset+widget->cursorWidth)<widget->geometry.height)
                    {
                        //显示下侧
                        llFillSingleColor(pos.x,pos.y+offset+widget->cursorWidth,pos.x+widget->geometry.width-1,pos.y+widget->geometry.height-1,widget->foregroundColor);
                    }
                    //显示游标
...
bool slotSliderMove(llConnectInfo info)
{
    ...
    if(llClickGetPoint(&x,&y)==true)
    {
        ...
        {
            //计算游标偏移值
            offset=x-(widget->geometry.x+globalPos.x);
...
//                }
        ...
        {
            //计算游标偏移值
            offset=y-(widget->geometry.y+globalPos.y);
        ...
        if(widget->movePos!=offset)
        {
            ...
            if(widget->isAutoMove)
            {
                ...
                else
                {
                    ...
                    widget->percent=100-u32Temp/maxLength;
                }
                              ...
                              bool isColor,bool isHorizontal,bool isHidden)
{
    ...
    if(llList_GetInfoByName(&parentInfo,parentNameId)==true)
    {
        ...
        if(pNewWidget!=NULL)
        {
            ...
            pNewWidget->percent=(isHorizontal?0:100);
...
void pSliderSetPercent(llSlider* widget,uint8_t percent,bool isEmit)
{
    float fTemp=0;
    ...
    if(widget->isEnable)
    {
        ...
        if(percent!=widget->percent)
        {
            ...
            {
                fTemp=(widget->geometry.height-widget->cursorWidth)/100.0;
                fTemp=fTemp*(100-percent);
                widget->movePos=fTemp;
                if(widget->movePos>(widget->geometry.height+widget->geometry.y-widget->cursorWidth))
                {
                    widget->movePos=widget->geometry.height+widget->geometry.y-widget->cursorWidth;
                }
            }
            pSliderRefresh(widget);
...
void nSliderSetPercent(uint16_t nameId,uint8_t percent,bool isEmit)
{
    llSlider* widget;
    widget=llGeneralGetWidget(nameId);
    if(widget!=NULL)
    {
        pSliderSetPercent(widget,percent,isEmit);
    }
}
