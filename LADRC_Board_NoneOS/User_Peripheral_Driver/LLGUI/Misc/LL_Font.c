        ...
        for(i=0;i<userFontLibTotalNum;i++)
        {
            if((llStringCompare((uint8_t *)userFontLibList[i].name,fontName)==true)&&(userFontLibList[i].fontSize==fontSize))
            {
                ...
                case typeBinFile:
                {
                    ...
                    activeFontInfo.libType=userFontLibList[i].libType;
                    activeFontInfo.pointer=userFontLibList[i].pointer;
                    llZkBinFontOpenLibrary(activeFontInfo.pointer);
                    break;
                }
                default:
                    break;
...
void llFontGetTinyData(uint32_t charAddr,uint8_t* outBuf,uint16_t byteCount)
{
    if(llFontGetLibType()==typeBinFile)
    {
//        llZkBinGetTinyData(allBitNum,byteNum,word,dataBits,width,height,outBuf,byteCount);

        //读固定长度数据
        llReadExFlash(charAddr+byteCount,outBuf,CHAR_BUFFER_BYTE_MAX);
    }
}
...
uint8_t* llFontGetData(uint8_t *word,uint8_t *dataBits, uint16_t *width, uint16_t *height,uint8_t* outBuf,uint32_t *outAddr,uint16_t byteCount)
{
    uint8_t *buffer=NULL;
    ...
    case typeBinFile:
    {
        llZkBinFontGetData(word,dataBits,outAddr,width,height);

        //读固定长度数据
        llReadExFlash(*outAddr+byteCount,outBuf,CHAR_BUFFER_BYTE_MAX);

//        uint16_t norLen=(*width)*(*height)/(8/(*dataBits));
//        if((*width)*(*height)%(8/(*dataBits)))
//        {
//            norLen++;
//        }
//        llReadExFlash(addr+byteCount,outBuf,norLen);
        buffer=outBuf;
        break;
    }
    default:
...
llFontLibType llFontGetLibType(void)
{
    return activeFontInfo.libType;
}
...
uint8_t llFontGetWidthSize(uint8_t * word)
{
    ...
    case typeBinFile:
    {
        width=llZkBinFontGetWidthSize(word);
        break;
    }
    default:
        break;
...
uint8_t llFontGetHeightSize(void)
{
    ...
    case typeBinFile:
    {
        width=llZkBinFontGetHeightSize();
        break;
    }
    default:
        break;
